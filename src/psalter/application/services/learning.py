...
class LearningService:
    ...
    def begin_or_resume(self, passage_id: str) -> LearningSessionDTO:
        ...
        if session is None:
            ...
            session = LearningSession(
                id=str(uuid4()), passage_id=passage_id, phase=LearningPhase.UNSEEN,
                practice_level=0, successful_blank_recitations=0,
    ...
    def _complete_exposure(self, passage_id: str, *, skip_practice: bool) -> LearningSessionDTO:
        ...
        try:
            updated = session.complete_exposure(self._clock.now(), skip_practice=skip_practice)
        except InvalidTransitionError as exc:
            raise InvalidLearningTransitionError(str(exc)) from exc
        self._sessions.upsert(updated)
        return _to_dto(updated)

    def submit_shadow_typing(self, passage_id: str, text: str) -> ShadowTypingResultDTO:
        view = self.get_learning_view(passage_id)
        if view.session.phase is not LearningPhase.PRACTICE or view.session.practice_level != 0:
            raise InvalidLearningTransitionError("Shadow typing is only available at practice level 0")
        expected = _copywork_tokens(view.passage.canonical_text)
        received = _copywork_tokens(text)
        if expected == received:
            self.complete_practice_level(passage_id)
            return ShadowTypingResultDTO(accepted=True)
        mismatch_index = next(
            (index for index, pair in enumerate(zip(expected, received, strict=False)) if pair[0] != pair[1]),
            min(len(expected), len(received)),
        )
        start = max(0, mismatch_index - 3)
        end = min(len(expected), mismatch_index + 5)
        return ShadowTypingResultDTO(
            accepted=False,
            mismatch_excerpt=" ".join(expected[start:end]) or "the beginning of the passage",
        )
    ...
    def get_passage(self, passage_id: str) -> PassageDetailDTO:
        ...
        return PassageDetailDTO(
            id=passage.id, psalm_id=passage.psalm_id,
            translation_id=passage.translation_id, psalm_number=passage.psalm_number,
            start_verse=passage.start_verse, end_verse=passage.end_verse,
            canonical_text=passage.canonical_text, sequence_number=passage.sequence_number,
            kind=passage.kind, segmentation_policy_version=passage.segmentation_policy_version,
        )

    def get_passage_summaries(self) -> list[PassageSummaryDTO]:
        return [
            PassageSummaryDTO(
                id=item.id, psalm_id=item.psalm_id, translation_id=item.translation_id,
                psalm_number=item.psalm_number, start_verse=item.start_verse,
                end_verse=item.end_verse, sequence_number=item.sequence_number, kind=item.kind,
            ) for item in self._passages.list_all()
        ]

    def get_learning_view(self, passage_id: str) -> LearningViewDTO:
        return LearningViewDTO(passage=self.get_passage(passage_id), session=_to_dto(self._require_session(passage_id)))

    def get_practice_view(self, passage_id: str, *, mismatch_excerpt: str | None = None) -> PracticeViewDTO:
        view = self.get_learning_view(passage_id)
        if view.session.practice_level == 0:
            return PracticeViewDTO(
                session=view.session, kind=PracticeKind.SHADOW_TYPING,
                canonical_text=view.passage.canonical_text, masked_text=None,
                level=0, max_level=self._max_practice_level,
                mismatch_excerpt=mismatch_excerpt,
            )
        return PracticeViewDTO(
            session=view.session, kind=PracticeKind.MASKED_RECALL,
            canonical_text=None,
            ...
            level=view.session.practice_level, max_level=self._max_practice_level,
    ...
    def _require_session(self, passage_id: str) -> LearningSession:
        session = self._sessions.get_by_passage(passage_id)
        if session is None:
            raise LearningSessionNotFoundError(f"Learning session not found for passage: {passage_id}")
        return session


def _copywork_tokens(text: str) -> tuple[str, ...]:
    return tuple(match.group(0).casefold().replace("’", "'") for match in _WORD_PATTERN.finditer(text))


def _to_dto(session: LearningSession) -> LearningSessionDTO:
    return LearningSessionDTO(
        id=session.id, passage_id=session.passage_id, phase=session.phase,
